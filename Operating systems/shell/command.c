...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    /* total_size is a length of PATH, offset is uor current position in PATH */
    size_t total_size = strlen(path), offset = 0;
    while (offset < total_size) {
      /* current_length is a length of current path */
      size_t current_length = strcspn(path + offset, ":");
      char *current_path = strndup(path + offset, current_length);
      strapp(&current_path, "/");
